 ...
 */
@CoordinatorLayout.DefaultBehavior(SmoothAppBarLayoutOld.Behavior.class)
public class SmoothAppBarLayoutOld extends AppBarLayout {
  ...
  public SmoothAppBarLayoutOld(Context context) {
    super(context);
    init(null);
  }

  public SmoothAppBarLayoutOld(Context context, AttributeSet attrs) {
    super(context, attrs);
    init(attrs);
  }
  ...
  public void syncOffsetDelayed(int delayMillis) {
    ...
    mHandler.postDelayed(new Runnable() {
      @Override
      public void run() {
        SmoothAppBarLayoutOld.this.syncOffset();
      }
    }, delayMillis);
  }
  ...
  public static class Behavior extends AppBarLayout.Behavior {
    ...
    protected void dispatchOffsetUpdates(AppBarLayout layout, int translationOffset) {
      if (layout instanceof SmoothAppBarLayoutOld) {
        List listeners = ((SmoothAppBarLayoutOld) layout).mOffsetChangedListeners;
        int i = 0;
    ...
    protected boolean init(final CoordinatorLayout coordinatorLayout, final AppBarLayout child, final View target) {
      if (mOnOffsetSyncedListener == null && child instanceof SmoothAppBarLayoutOld) {
        ...
        ((SmoothAppBarLayoutOld) child).addOnOffsetSyncedListener(mOnOffsetSyncedListener);
      }

      if (mOnPageChangeListener == null && child instanceof SmoothAppBarLayoutOld) {
        ...
        vViewPager = ((SmoothAppBarLayoutOld) child).getViewPager();
        if (vViewPager != null) {
    ...
    protected void scrolling(CoordinatorLayout coordinatorLayout, AppBarLayout child, View target, int offset) {
      ...
      if (child instanceof SmoothAppBarLayoutOld && ((SmoothAppBarLayoutOld) child).mHaveChildWithInterpolator) {
      ...
      if (layout != null) {
        ...
        for (int z = layout.getChildCount(); i < z; ++i) {
          ...
          if (layoutParams instanceof LayoutParams) {
            LayoutParams childLp = (LayoutParams) layoutParams;
